...
test('victory rewards enter the bag once and can be equipped without discarding the replaced die', async ({ page }) => {
  ...
  await equipItem(page, 'die', 0, DIE_DEFINITIONS.find((d) => d.name === rewardName)!.id)
...
test('a third badge is retained in the bag and can replace an equipped badge before entry', async ({ page }) => {
  ...
  await equipItem(page, 'badge', 1, 'lucky-charm')
  await page.getByRole('button', { name: '入座，开始这一桌', exact: true }).click()
...
test('keyboard-only route covers opening core, equipment, selection, reward, pause and Continue', async ({ page }, info) => {
  ...
  await page.keyboard.press('Enter')
  const badge = page.getByRole('button', { name: /^徽章 1：/ })
  await tabTo(page, badge); await page.keyboard.press('e')
  await tabTo(page, page.locator('.bag-list [data-item-id=""][data-item-kind="badge"]')); await page.keyboard.press('e')
  await expect(badge).toHaveAttribute('aria-label', /空位/)
  await tabTo(page, badge); await page.keyboard.press('e')
  await tabTo(page, page.locator('.bag-list [data-item-id="core-kindred"]')); await page.keyboard.press('e')
  await expect(badge).toHaveAttribute('aria-label', /同契纹章/)
  ...
  await page.keyboard.press('Enter')
  const slot = page.getByRole('button', { name: /^骰子 1：/ })
  await tabTo(page, slot); await page.keyboard.press('e')
  await tabTo(page, page.locator('.bag-list [data-item-id="' + DIE_DEFINITIONS.find((d) => d.name === rewardName)!.id + '"]')); await page.keyboard.press('e')
  await expect(slot).toHaveAttribute('aria-label', new RegExp(rewardName))
  await tabTo(page, page.getByRole('button', { name: '入座，开始这一桌', exact: true }))
  await page.keyboard.press('Enter')
...
test('a first ordinary loss retries the same opponent with all gear and no reward', async ({ page }) => {
  ...
  await equipItem(page, 'badge', 0, 'core-steady')
  await page.getByRole('button', { name: '入座，开始这一桌', exact: true }).click()
