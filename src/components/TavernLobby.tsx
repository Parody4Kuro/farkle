...
export function TavernLobby({ saved, profile, comfort, onStart, onContinue, onClassic, onComfort, warning }: {
  ...
  return <main className={`tavern-lobby ${comfort.largeText ? 'large-text' : ''}`}>
    ...
    <section className="lobby-copy">
      ...
      <p className="lobby-meta">四桌冒险 · 约 20–30 分钟 · 随时离席续玩</p>
    ...
    <footer className="lobby-footer"><span>原创骰组冒险 / 完全本地运行</span><span>{profile.nights ? `${profile.nights} 夜故事 · ${profile.wins} 次通关 · 最高落袋 ${profile.peak}` : '先赢下一桌，再决定下一步。'}</span></footer>
