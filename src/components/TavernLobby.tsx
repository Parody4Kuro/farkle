...
export function TavernLobby({ saved, profile, comfort, onStart, onContinue, onClassic, onFriends, onComfort, warning }: {
  ...
  return <main className={`tavern-lobby ${comfort.largeText ? 'large-text' : ''}`}>
    ...
    <section className="lobby-copy">
      ...
      <div className="lobby-links"><button onClick={onClassic}>经典对局</button>{onFriends && <><span>·</span><button onClick={onFriends}>好友对战</button></>}<span>·</span><button onClick={() => setMemories(!memories)} aria-expanded={memories}>酒馆记忆 {profile.memories.length}/4</button></div>
    ...
    <footer className="lobby-footer"><span>原创骰组冒险 / 单机与好友直连</span><span>{profile.nights ? `${profile.nights} 夜故事 · ${profile.wins} 次通关 · 最高落袋 ${profile.peak}` : '先选一枚核心，再开始今晚的故事。'}</span></footer>
