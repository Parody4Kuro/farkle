...
/** Saved runs keep this version. Add a new table instead of rewriting a released version. */
export const SCORING_VERSION = 1

function coreScore(group: ScoreGroup, version: number, scale: (group: ScoreGroup) => number): number {
  if (version !== SCORING_VERSION) throw new RangeError('Unsupported scoring version')
  return group.score * scale(group)
}

export const CORE_MODIFIERS: GameModifier[] = [
  {
